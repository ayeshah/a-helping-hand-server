...
module.exports = async function profile(req, res) {
	try{
        ...
        const [rows] = await db.connection.query("SELECT `is_verified`,`bio` FROM `recipient` WHERE `recipient_id` = ?;", [req.params.id]);
        let is_verified = rows[0].is_verified
        let bio = rows[0].bio
        console.debug("Recipient is_verified = %s", is_verified)
        console.debug("Recipient bio = %s", bio)
	    res.send({code: "Success", bio: bio, is_verified: is_verified})
 	}
 	catch(err){
        console.log(err)
